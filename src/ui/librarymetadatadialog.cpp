...
LibraryMetadataDialog::LibraryMetadataDialog(QWidget *parent) : QDialog(parent) {
	...
	m_idEdit->setObjectName(QStringLiteral("libraryId"));
	...
	m_form->addRow(tr("バージョン:"), m_versionEdit);
	m_form->addRow(tr("作者:"), m_authorEdit);
	m_form->addRow(tr("作者URL:"), m_authorUrlEdit);
	m_form->addRow(tr("ホームページ:"), m_homepageEdit);
	m_form->addRow(tr("説明:"), m_descriptionEdit);
	m_form->addRow(tr("ライセンス:"), m_licensePicker);
...
void LibraryMetadataDialog::setMode(Mode mode) {
	...
	if (mode == Mode::Create) {
		...
		m_form->setRowVisible(m_idEdit, true);
	} else {
		setWindowTitle(tr("ライブラリのメタデータを編集"));
		m_buttons->button(QDialogButtonBox::Ok)->setText(tr("保存"));
		m_idEdit->setEnabled(false);
		// 単独のメタデータ編集は id 欄なしのシンプルなフォームにする (仕様書§14。id は
		// 複製時のみ入力対象で、既存ライブラリの id は編集不可なので表示する意味が薄い)。
		m_form->setRowVisible(m_idEdit, false);
	}
}
